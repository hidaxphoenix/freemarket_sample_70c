...
function appendChidrenBox(insertHTML){
  var childSelectHtml = '';
  childSelectHtml = `
                      <div class='main__box__detail__category__form__box'  id= 'children_wrapper_edit2'>
                        <select class="listing-select-wrapper__box--select" id="child_category_edit2" name="">
...
function appendGrandchidrenBox(insertHTML){
  ...
  grandchildSelectHtml = `<div class='main__box__detail__category__form__box' id= 'grandchildren_wrapper_edit2'>
                            <select class="listing-select-wrapper__box--select" id="grandchild_category_edit2" name="category_id">
                              <option value="---" data-category="---">---</option>
                                ${insertHTML}
                            </select>
                          </div>`;
  ...
  // 子カテゴリー選択後のイベント
  $('.main__box__detail__category__form').on('change', '#child_category_edit2', function(){
    var childId = $('#child_category_edit2 option:selected').data('category'); //選択された子カテゴリーのidを取得
    ...
    // 子カテゴリー2選択後のイベント
    $('.main__box__detail__category__form').on('change', '#child_category_edit', function(){
      var childId2 = $('#child_category_edit option:selected').data('category'); //選択された子カテゴリーのidを取得
      if (childId2 != "---"){ //子カテゴリーが初期値でないことを確認
        $.ajax({
          url: 'get_category_grandchildren',
          type: 'GET',
          data: { child_id: childId2 },
          dataType: 'json'
        })
        .done(function(grandchildren){
          if (grandchildren.length != 0) {
